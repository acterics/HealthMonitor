...
import com.acterics.healthmonitor.stompclient.frame.FrameBodyConverter;
import com.acterics.healthmonitor.stompclient.frame.GsonFrameBodyConverter;
import com.acterics.healthmonitor.stompclient.listener.ConnectedListnener;
import com.acterics.healthmonitor.stompclient.listener.DisconnectListener;
import com.acterics.healthmonitor.stompclient.listener.ErrorListener;
import com.acterics.healthmonitor.stompclient.listener.ReceiptListener;
import com.acterics.healthmonitor.stompclient.listener.SubscribeHandler;
import com.acterics.healthmonitor.stompclient.listener.SubscribeHandlerWithoutPayload;
...
import com.acterics.healthmonitor.stompclient.websocketClient.DefaultWebsocketClient;
import com.acterics.healthmonitor.stompclient.websocketClient.WebsocketEventHandler;
...
//	private WebSocketClient webSocketClient;
	...
	private void transmit(Command command, Map<String, String> headers, String body) {
		if (!connected) {
			if (!Command.CONNECT.equals(command)) {
				Timber.d("not connected yet... command: {}", command);
				return;
			}
		}
	...
	private void setupHeartbeat(StompHeaders headers) {
		...
		if (!(heartbeat.getOutgoing() == 0 || serverIncoming == 0)) {
			...
			Timber.d("send PING every {}ms", ttl);
			
			
			//pinger 설정
			pinger = new Timer();
			pinger.scheduleAtFixedRate(new TimerTask() {
//				Timber Timber = TimberFactory.getTimber(getClass());
				...
				public void run() {
					try {
						...
						Timber.e("ping send fail. not yet connected.");
					}
				}
			}, ttl, ttl);
		}
		...
		if (!(heartbeat.getIncoming() == 0 || serverOutgoing == 0)) {
			...
			Timber.d("check PONG every {}ms", ttl);
...
//				Timber Timber = TimberFactory.getTimber(getClass());
				...
				public void run() {
					...
					if (delta > ttl * 2) {
						...
						Timber.d("did not receive server activity for the last {}ms", delta);
	...
	public StompClient connect(final ConnectedListnener connectedLintener) {
		...
		Timber.d("Opening Web Socket... url: {}", url);
				...
				if (Command.CONNECTED.equals(frame.getCommand())) {
					...
					if (connectedLintener != null) {
						try {
							...
							e.printStackTrace();
//							Timber.e("onConnected e url: " + url, e);
						}
					} else {
						Timber.d("Unhandled received CONNECTED: {}", frame);
					}
...
//					}
					...
					if (subscription != null) {
						...
						if (subscriptionListener != null) {
							...
							try {
								...
								Timber.e("onMessage e subscrition id: " + subscriptionId, e);
							}
							...
							Timber.d("Unhandled received MESSAGE: {}", frame);
						}
					} else {
						Timber.d("subscription id not found. subscrition id: {}", subscriptionId);
					}
					...
					if (receipt != null) {
						...
						if (receiptListener != null) {
							try {
								...
								Timber.e("receiptListener.onReceipt e. receipt id: " + receiptId, e);
							}
						} else {
							Timber.d("Unhandled received RECEIPT: " + frame);
						}
					} else {
						...
						if (globalReceiptListener != null) {
							try {
								...
								Timber.e("globalReceiptListener.onReceipt e. receipt id: " + receiptId, e);
							}
						} else {
							Timber.d("Unhandled received RECEIPT: {}", frame);
						}
					...
					if (errorListener != null) {
						try {
							...
							Timber.e("onErrorCallback e. frame: " + frame, e);
						}
					} else {
						Timber.d("Unhandled received ERROR: {}", frame);
					}
				} else {
					Timber.d("Unhandled frame: {}", frame);
				}
			}
			...
			public void onClose(int code, String reason, boolean remote) {
				if (code == 1000) {
					...
					Timber.d("Whoops! Lost connection to {}", connection.getUrl());
				}
				...
				if (webSocketCloseListener != null) {
					try {
						...
						Timber.e("webSocketCloseListener.onClose() e. code: " + code +
								", reason: " + reason +
								", remote: " + remote,
								e);
					}
				} else {
					Timber.d("Unhandled onClose event. code: {}, reason: {}, remote: {}", code, reason, remote);
				}
				...
				if (disconnectListener != null) {
					try {
						...
						Timber.e("disconnectListener.onDisconnect() e.", e);
					}
				}
			}
			...
			public void onError(Exception ex) {
				Timber.e("websocket e", ex);
				if (webScoketErrorListener != null) {
					try {
						webScoketErrorListener.onError(ex);
					} catch (Exception e) {
						Timber.e("webScoketErrorListener.onError() e.", e);
					}
				} else {
					Timber.d("Unhandled onError event.", ex);
				}
			}
		};
	}
...
//	}
	...
	public void unsubscribe(String id) {
		...
		if (subscription == null) {
			Timber.d("not exist subscription id. id: {}", id);
